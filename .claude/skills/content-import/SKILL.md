...
1. **Check the tier.** A Tier C type is one line in the `KINDS` of `load/entities.ts`
   and nothing else, and those rows are searchable and resolve tags. Take the order
   below only when the sheet queries specific columns, which is what Tier A buys.
